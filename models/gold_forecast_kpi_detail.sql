...
SELECT
    ...
    (SELECT seq FROM {{ ref('dim_current_period') }}) - cur.seq AS periods_ago,
    cur.channel,
    cur.nsm_id, cur.nsm_name,
    cur.grsm_id, cur.grsm_name,
    cur.rsm_id, cur.rsm_name,
    cur.ss_id, cur.ss_name,
    cur.sbu_id, cur.sbu_name,
    cur.brand_id, cur.brand_name,
    cur.subbrand_id, cur.subbrand_name,
    cur.parent_id, cur.parent_name,
    cur.total_forecast,
    cur.total_actual,
...
LEFT JOIN agg_seq prev
    ...
    -- 9-key join (semua dimensi id): mencegah fan-out dari entity
    -- dengan multi-mapping (mis. 1 parent -> 2 subbrand_id).
    -- IS NOT DISTINCT FROM = NULL-safe equality (NULL match NULL).
