...
aggregated_flat_data AS (
    -- 🗜️ 2. MERAPATKAN DATA CY & LY MENJADI SATU BARIS HORIZONTAL (ANTI-NOL)
    SELECT 
        ...
        -- Satukan jualan mingguan ke samping
        SUM(CASE WHEN flag = 'cy' THEN target_qty ELSE 0 END) AS target_qty_cy,
        SUM(CASE WHEN flag = 'cy' THEN stm_qty ELSE 0 END) AS stm_qty_cy,
        SUM(CASE WHEN flag = 'cy' THEN salfo_qty ELSE 0 END) AS salfo_qty_cy,
        
        SUM(CASE WHEN flag = 'cy' THEN target_value ELSE 0 END) AS target_val_cy,
        SUM(CASE WHEN flag = 'cy' THEN stm_value ELSE 0 END) AS stm_val_cy,
        SUM(CASE WHEN flag = 'cy' THEN salfo_value ELSE 0 END) AS salfo_val_cy,
...
base_data_ytd AS (
    -- 🌟 3. SEKARANG KITA GULUNG YTD & MTD SECARA HORIZONTAL BERSIH
    SELECT 
        a.*,
        -- Gulungan YTD (Current Year)
        ...
        -- Gulungan YTD (Last Year) -> 🔥 DIJAMIN MENGALIR KARENA SUMBUNYA SUDAH SATU BARIS
...
final_projections AS (
    SELECT 
        cl.*,
        CASE WHEN cl.week > 0 THEN ((cl.stm_qty_ytd + cl.salfo_qty_cy) / cl.week) * (52 - cl.week) ELSE 0 END AS est_stm_forward_qty,
        CASE WHEN cl.week > 0 THEN ((cl.stm_val_ytd + cl.salfo_val_cy) / cl.week) * (52 - cl.week) ELSE 0 END AS est_stm_forward_val
...
unpivoted AS (
    ...
    SELECT 
        ...
        stm_qty_ytd_ly AS stm_ytd_ly, -- 🔥 SEKARANG DIJAMIN ADA ANGKA NYATA NYA!
        ...
        (stm_qty_ytd + salfo_qty_cy + est_stm_forward_qty) AS stm_est_fy,
    ...
    SELECT 
        ...
        stm_val_ytd_ly AS stm_ytd_ly, -- 🔥 SEKARANG DIJAMIN ADA ANGKA NYATA NYA!
        ...
        (stm_val_ytd + salfo_val_cy + est_stm_forward_val) AS stm_est_fy,
