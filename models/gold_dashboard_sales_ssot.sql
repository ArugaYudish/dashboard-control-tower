...
WITH current_operational AS (
    -- 📅 1. KUNCIAN KALENDER OPERASIONAL (Murni Numeric mengikuti skema m_cycle3)
    SELECT 
        year AS cur_year,
...
base_data AS (
    SELECT 
        ...
        -- =========================================================================
        -- 🌟 4. GULUNG MENTOK TAHUNAN W1-W52 (TOTAL TARGET & FORECAST TAHUNAN)
        -- =========================================================================
...
calculated_features AS (
    SELECT 
        b.*,
        -- 5. Ambil data Last Month (LM) mundur 4 minggu untuk kebutuhan performa pasar
        ...
        -- 6. FORECAST KEDEPAN = TOTAL FORECAST TAHUNAN - FORECAST YTD YANG SUDAH TERLEWATI
...
final_projections AS (
    SELECT 
        cl.*,
        -- 7. EKSEKUSI RUMUS RESMI: ((STM YTD + SALFO KEDEPAN) / CURRENT WEEK) * SISA WEEK
        CASE 
            WHEN cl.week > 0 THEN ((cl.stm_qty_ytd + cl.salfo_qty_kedepan) / cl.week) * (52 - cl.week)
            ELSE 0 
        END AS est_stm_forward_qty,
        
        CASE 
            WHEN cl.week > 0 THEN ((cl.stm_val_ytd + cl.salfo_val_kedepan) / cl.week) * (52 - cl.week)
            ELSE 0 
...
unpivoted AS (
    ...
    SELECT 
        ...
        -- Atribut Helper UI Superset (Murni numeric bertemu numeric)
