...
omset_ibn as (
  select year, week, pcode, distributor_id, sta_qty, sta_value
  from spx.m_sta_subdist
),
avgs_ibn as (
  select oi.pcode, oi.distributor_id,
  ...
  group by oi.pcode, oi.distributor_id
)
select md.sls_div as channel, cast(voswb.tahun as numeric) as year, voswb.periode as period, to_char(to_date(cast(voswb.periode as text), 'MM'), 'Mon') as periodName, cast(voswb.week as numeric) as week,
       ...
       a.avg_13w_qty, a.avg_13w_value, aibn.avg_5w_sta_qty, aibn.avg_5w_sta_value, now() as loaded_at
...
left join spx.v_sales_hierarchy_product vsh on voswb.distributor_id = vsh.distributor_id and voswb.pcode = vsh.pcode
...
left join spx.v_stock_dist sss
  on cast(voswb.tahun as int) = sss.year and cast(voswb.week as int) = sss.week
  and voswb.pcode = sss.pcode and voswb.distributor_id = sss.sub_id
...
left join omset_ibn oi
  ...
  and voswb.pcode = oi.pcode and voswb.distributor_id = oi.distributor_id
left join avgs a
  on a.distributor_id = voswb.distributor_id and a.pcode = voswb.pcode
left join avgs_ibn aibn on voswb.pcode = aibn.pcode and voswb.distributor_id = aibn.distributor_id
