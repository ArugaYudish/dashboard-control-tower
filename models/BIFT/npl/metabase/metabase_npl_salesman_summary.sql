-- Metabase SQL Query: NPL Breakdown Summary by Salesman
-- Source: bift.gold_npl_by_salesman_dev (or bift.gold_npl_by_salesman in production)
-- Output: SD | NSM | GRSM | RSM | SS | Distributor | Break By (sls_id - sls_nm) | cb_cover | oa | %oa | total_dropsize | non_repeat | t2 | t3 | t4 | t5 | t6 | percent_repeat
...
WITH outlet_orders AS (
    ...
    FROM bift.gold_npl_by_salesman_dev
    WHERE 1=1
      ...
      [[ AND ss_id IN ({{ss_ids}}) ]]
      [[ AND sls_id IN ({{slsman_ids}}) ]]

    GROUP BY ss_id, distributor_id, sls_id, sls_nm, cust_id
)
...
FROM outlet_orders
GROUP BY ss_id, distributor_id, sls_id, sls_nm
ORDER BY ss_id, distributor_id, sls_id;
