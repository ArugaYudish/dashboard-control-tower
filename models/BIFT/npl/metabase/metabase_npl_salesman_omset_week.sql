-- Metabase SQL Query: NPL Breakdown Weekly Omset by Salesman
-- Source: bift.gold_npl_by_salesman_dev (or bift.gold_npl_by_salesman in production)
-- Output: SD | NSM | GRSM | RSM | SS | Distributor | Break By (sls_id - sls_nm) | week | omset
...
WITH outlet_orders AS (
    ...
    GROUP BY ss_id, distributor_id, sls_id, sls_nm, week
)
...
FROM outlet_orders
GROUP BY ss_id, distributor_id, sls_id, sls_nm, week
ORDER BY ss_id, distributor_id, sls_id, week;
