-- Metabase SQL Query: NPL Breakdown Weekly Omset by Channel
-- Source: bift.gold_npl_by_channel_dev (or bift.gold_npl_by_channel in production)
-- Output: SD | NSM | GRSM | RSM | SS | Distributor | Break By (channel_id - channel_nm) | week | omset
...
WITH outlet_orders AS (
    ...
    GROUP BY ss_id, distributor_id, channel_id, channel_nm, week
)
...
FROM outlet_orders
GROUP BY ss_id, distributor_id, channel_id, channel_nm, week
ORDER BY ss_id, distributor_id, channel_id, week;
