-- Metabase SQL Query: NPL Breakdown Summary by Salesforce
-- Source: bift.gold_npl_by_salesforce_dev (or bift.gold_npl_by_salesforce in production)
-- Output: SD | NSM | GRSM | RSM | SS | Distributor | Break By (salesforce_id - salesforce_nm) | cb_cover | oa | %oa | total_dropsize | non_repeat | t2 | t3 | t4 | t5 | t6 | percent_repeat
...
WITH outlet_orders AS (
    ...
    GROUP BY ss_id, distributor_id, salesforce_id, salesforce_nm, cust_id
)
...
FROM outlet_orders
GROUP BY ss_id, distributor_id, salesforce_id, salesforce_nm
ORDER BY ss_id, distributor_id, salesforce_id;
