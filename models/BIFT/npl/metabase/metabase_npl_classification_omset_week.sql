-- Metabase SQL Query: NPL Breakdown Weekly Omset by Classification
-- Source: bift.gold_npl_by_classification_dev (or bift.gold_npl_by_classification in production)
-- Output: SD | NSM | GRSM | RSM | SS | Distributor | Break By (classification_id - classification_nm) | week | omset
...
WITH outlet_orders AS (
    ...
    GROUP BY ss_id, distributor_id, classification_id, classification_nm, week
)
...
FROM outlet_orders
GROUP BY ss_id, distributor_id, classification_id, classification_nm, week
ORDER BY ss_id, distributor_id, classification_id, week;
