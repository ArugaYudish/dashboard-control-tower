-- Metabase SQL Query: NPL Breakdown Summary by Group Channel
-- Source: bift.gold_npl_by_group_channel_dev (or bift.gold_npl_by_group_channel in production)
-- Output: SD | NSM | GRSM | RSM | SS | Distributor | Break By (group_channel_id - group_channel_nm) | cb_cover | oa | %oa | total_dropsize | non_repeat | t2 | t3 | t4 | t5 | t6 | percent_repeat
...
WITH outlet_orders AS (
    ...
    GROUP BY ss_id, distributor_id, group_channel_id, group_channel_nm, cust_id
)
...
SELECT
    ...
    -- Break By Column (group_channel_id - group_channel_nm)
    CASE 
        WHEN group_channel_id IS NOT NULL 
        THEN CONCAT(group_channel_id, ' - ', group_channel_nm)
        ELSE NULL 
    END                                                                 AS break_by_group_channel,
...
FROM outlet_orders
GROUP BY ss_id, distributor_id, group_channel_id, group_channel_nm
ORDER BY ss_id, distributor_id, group_channel_id;
