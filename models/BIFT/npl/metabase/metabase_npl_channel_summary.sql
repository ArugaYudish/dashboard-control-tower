-- Metabase SQL Query: NPL Breakdown Summary by Channel
-- Source: bift.gold_npl_by_channel_dev (or bift.gold_npl_by_channel in production)
-- Output: SD | NSM | GRSM | RSM | SS | Distributor | Break By (channel_id - channel_nm) | cb_cover | oa | %oa | total_dropsize | non_repeat | t2 | t3 | t4 | t5 | t6 | percent_repeat
...
WITH outlet_orders AS (
    ...
    FROM bift.gold_npl_by_channel_dev
    WHERE 1=1
    ...
    GROUP BY ss_id, distributor_id, channel_id, channel_nm, cust_id
)
...
SELECT
    ...
    -- Break By Column (channel_id - channel_nm)
    CASE 
...
FROM outlet_orders
GROUP BY ss_id, distributor_id, channel_id, channel_nm
ORDER BY ss_id, distributor_id, channel_id;
