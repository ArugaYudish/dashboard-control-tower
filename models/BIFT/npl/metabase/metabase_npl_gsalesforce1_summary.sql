-- Metabase SQL Query: NPL Breakdown Summary by Group Salesforce 1 (gsalesforce1)
-- Source: bift.gold_npl_by_gsalesforce1_dev (or bift.gold_npl_by_gsalesforce1 in production)
-- Output: SD | NSM | GRSM | RSM | SS | Distributor | Break By (gsalesforce1_id - gsalesforce1_nm) | cb_cover | oa | %oa | total_dropsize | non_repeat | t2 | t3 | t4 | t5 | t6 | percent_repeat
...
WITH outlet_orders AS (
    ...
    GROUP BY ss_id, distributor_id, gsalesforce1_id, gsalesforce1_nm, cust_id
)
...
SELECT
    ...
    -- Break By Column (gsalesforce1_id - gsalesforce1_nm, NULL for M1 & M3)
    CASE 
...
ORDER BY ss_id, distributor_id, gsalesforce1_id;
