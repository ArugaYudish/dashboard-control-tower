{{
    config(
        schema='bift',
        materialized='table',
        alias='gold_npl_by_classification_dev',
...
-- DEV/TESTING ONLY: Gold table grouped by classification (M2 only, NULL for M1 & M3)
...
-- STREAM B: Real Transaction Rows from Silver (Aggregated per outlet + pcode + week x classification)
SELECT
    t.source_schema,
    t.tahun,
    t.periode,
